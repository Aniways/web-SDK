...
function Configuration(){
  ...
  function configure(responseText){
    ...
    if(!storedConfiguration || newConfiguration.version > storedConfiguration.version){
      $.extend(currentConfiguration, defaultConfiguration, newConfiguration, _userConfiguration);
      localStorage.setItem("aniwaysConfiguration", JSON.stringify(currentConfiguration));
    } else {
      $.extend(currentConfiguration, defaultConfiguration, storedConfiguration, _userConfiguration);
    }
  }

  function ajaxError(){
    console.error('There was a problem with the configuration request. Returned code was ' + this.status);
  }
  ...
  this.isContextual = function(){
    return currentConfiguration.AWcontextual;
  };

  this.isAnalytics = function(){
    return currentConfiguration.AWanalytics;
  };

  this.inputImageSize = function(){
    return currentConfiguration.inputImageSize;
  };

  this.popoverImageSize = function(){
    return currentConfiguration.popoverImageSize;
  };

  this.wallImageSize = function(){
...
window.Aniways = (function(){
  ...
  if(userId === null){
    userId = guid();
    localStorage.setItem("aniwaysUserId", userId);
  }
  ...
  function init(appId, userConfiguration){
    ...
    if (document.getElementsByClassName('aniways-wall').length > 0){
      ...
      wallObserver = new WallObserver(document.getElementsByClassName('aniways-wall'), decoder);
  ...
  function encodeText(nodeToEncode){
    ...
    analytics.messageAnalytics(userId, currentMessageID, mappingVersion, eventData);
    return encodedText;
  }
  ...
  function collectEventData(aniwaysDiv, encodedText){
    var eventData = {iconData:[], suggestData: [], message: encodedText};
    $(aniwaysDiv).children('.aniways-image').each(function(index, image){
      eventData.iconData.push({phrase: image.attributes['data-phrase'].value, iconName: image.src.replace(/^.*[\\\/]/, ''), partToReplace: image.attributes['data-phrase'].value });
      eventData.suggestData.push({phrase: image.attributes['data-phrase'].value, partToReplace: image.attributes['data-phrase'].value });
    });
    $(aniwaysDiv).children('.aniways-highlight').each(function(index, word){
      eventData.suggestData.push({phrase: word.attributes['data-phrase'].value, partToReplace: word.attributes['data-phrase'].value });
    });
    return eventData;
  }
...
function Decoder(phraseMapping, assetsToUrls, configuration){
  ...
  function urlDecoding(message){
    ...
    var encodingData = extractUrlEncodingData(messageParts[1]);
    var count = 0;
  ...
  function extractUrlEncodingData(url) {
    url = url.replace(/&amp;/g, '&');
...
function Encoder(aniwaysDiv, mapping, configuration){
  ...
  this.encodeText = function encodeText(){
    ...
    function recEncodeText(node){
      var encodedMessage = "";
      var childNodes = [].slice.call($(node)[0].childNodes);
...
function Highlighter(mapping){
  ...
  this.highlight = function highlight (node) {
    ...
    for(i=0; i< childNodes.length; i++) {
      textChildNode = childNodes[i];
      if (textChildNode.nodeType === 3){
    ...
    function processText(childNode){
      var match;
...
function saveSelection(containerEl) {
  ...
  var length = 0;
  preSelectionRange.getNodes().forEach(function(node){
    if(node.tagName === "IMG"){
      length = length + 1;
    }else if(node.nodeType === 3){
      if(node === preSelectionRange.endContainer){
        length = length + preSelectionRange.endOffset;
      }else{
...
function restoreSelection(containerEl, savedSel) {
  ...
  while (!stop && (node = nodeStack.pop())) {
    ...
    if (node.nodeType === 3) {
      ...
      var childNode, index;
      if (!foundStart && savedSel.start > charIndex && savedSel.start <= nextCharIndex) {
        for(index = 0; i< childNodes.length; i++) {
          childNode = childNodes[i];
          if( childNode === node) {
            break;
          }
        ...
        range.setStart(node.parentElement, i + 1);
        range.setEnd(node.parentElement, i + 1);
        foundStart = true;
      }
      if (foundStart && savedSel.end === charIndex && savedSel.end === nextCharIndex) {
        for(index = 0; i< childNodes.length; i++) {
        ...
        range.setStart(node.parentElement, i);
        range.setEnd(node.parentElement, i);
        stop = true;
      }
      charIndex = nextCharIndex;
    }else {
...
function WallObserver(wall, decoder){
  ...
  function handleNode(node){
    if(typeof node.getElementsByClassName === 'function'){
      var message;
      if(node.className.indexOf('aniways-message') > -1){
        message = [node];
      } else {
...
function Analytics(appId, configuration){
  ...
  var Client = function (appId) {
    this.appId = appId;
    this.APIUrl = "https://aniways-analytics.s3.amazonaws.com/Web/" + appId + "/";
  };

  var client = new Client(appId);

  this.messageAnalytics = function messageAnalytics(userId, messageId, keywordsVersion, eventData){
    var evt = {}, fileName = "";
    evt.type = "track";
    evt.category = "Input";
    evt.userId = userId;
    ...
    evt.suggestEvent = suggestDataAnalytics(eventData.suggestData);
    fileName = toTimeStamp(new Date()) + "." + userId + ".json";
    client.uploadEvent(evt, fileName);
  };

  function messageDataAnalytics(messageText){
    var evt = {};
    evt.action = "message sent";
    evt.text = messageText;
    return evt;
  }

  function iconDataAnalytics(iconData){
    var sentIconData = [], i = 0, data;
    for(i=0; i < iconData.length; i++){
      var evt = {};
      data = iconData[i];
      evt.action = "sent";
      evt.pharse = data.phrase;
      evt.partToReplace = data.partToReplace;
      evt.iconName = data.iconName;
      evt.fromButton = false;
      sentIconData.push(evt);
    }
    return sentIconData;
  }

  function suggestDataAnalytics(suggestData){
    var sentSuggestData = [], i = 0, data;
    for(i=0; i < suggestData.length; i++){
      var evt = {};
      data = suggestData[i];
      evt.action = "suggested";
      evt.pharse = data.phrase;
      evt.partToReplace = data.partToReplace;
      sentSuggestData.push(evt);
    }
    return sentSuggestData;
  }

  this.tapAnalytics = function tapEvent(userId, messageId, keywordsVersion, tapData){
    var evt = {}, fileName = "";
    evt.type = "track";
    evt.category = "Input";
    evt.action = "tap";
  ...
  this.selectAnalytics = function selectEvent(userId, messageId, keywordsVersion, selectData){
    var evt = {}, fileName = "";
    evt.type = "track";
    evt.category = "Input";
    evt.action = "selected";
  ...
  function threeDigits(d) {
    if(0 <= d && d < 10) { return "00" + d.toString(); }
    if(10 <= d && d < 99) { return "0" + d.toString(); }
    return d.toString();
  }

  function toTimeStamp(date) {
    return date.getUTCFullYear() + "-" + twoDigits(1 + date.getUTCMonth()) + "-" + twoDigits(date.getUTCDate()) + "T" + twoDigits(date.getUTCHours()) + ":" + twoDigits(date.getUTCMinutes()) + ":" + twoDigits(date.getUTCSeconds()) + "." + threeDigits(date.getUTCMilliseconds());
  }
   ...
   * @param {Function} [error] Invoked on failure
   */
  Client.prototype.uploadEvent = function (event, fileName) {

    if(configuration.isAnalytics() === false){return;}

    var url = this.APIUrl + fileName;
